...
<div class="toolbar">
  ...
  <div class="toolbar-group">
    <button class="toolbar-btn" onclick={() => run((v) => undo(v))} title="Undo (Cmd+Z)" aria-label="Undo">
      {@render icon('<path d="M3 7v6h6" /><path d="M21 17a9 9 0 0 0-9-9 9 9 0 0 0-6 2.3L3 13" />')}
    </button>
    <button class="toolbar-btn" onclick={() => run((v) => redo(v))} title="Redo (Cmd+Shift+Z)" aria-label="Redo">
  ...
  <!-- Text formatting -->
  <div class="toolbar-group">
    <button class="toolbar-btn" onclick={() => run(toggleBold)} title="Bold (Cmd+B)" aria-label="Bold">
      {@render icon('<path d="M6 4h8a4 4 0 0 1 4 4 4 4 0 0 1-4 4H6z M6 12h9a4 4 0 0 1 4 4 4 4 0 0 1-4 4H6z" />')}
    </button>
    <button class="toolbar-btn" onclick={() => run(toggleItalic)} title="Italic (Cmd+I)" aria-label="Italic">
      {@render icon('<line x1="19" y1="4" x2="10" y2="4" /><line x1="14" y1="20" x2="5" y2="20" /><line x1="15" y1="4" x2="9" y2="20" />')}
    </button>
    <button class="toolbar-btn" onclick={() => run(toggleStrikethrough)} title="Strikethrough" aria-label="Strikethrough">
      {@render icon('<path d="M16 4H9a3 3 0 0 0-2.83 4" /><path d="M14 12a4 4 0 0 1 0 8H6" /><line x1="4" y1="12" x2="20" y2="12" />')}
    </button>
    <button class="toolbar-btn" onclick={() => run(toggleInlineCode)} title="Inline Code" aria-label="Inline code">
  ...
  <!-- Headings -->
  <div class="toolbar-group">
    <button class="toolbar-btn" onclick={() => run((v) => insertHeading(v, 1))} title="Heading 1" aria-label="Heading 1">
      <span class="toolbar-text">H<sub>1</sub></span>
    </button>
    <button class="toolbar-btn" onclick={() => run((v) => insertHeading(v, 2))} title="Heading 2" aria-label="Heading 2">
      <span class="toolbar-text">H<sub>2</sub></span>
    </button>
    <button class="toolbar-btn" onclick={() => run((v) => insertHeading(v, 3))} title="Heading 3" aria-label="Heading 3">
  ...
  <!-- Insert elements -->
  <div class="toolbar-group">
    <button class="toolbar-btn" onclick={() => run(insertLink)} title="Link" aria-label="Insert link">
    ...
    <button class="toolbar-btn" onclick={() => run(insertImage)} title="Image" aria-label="Insert image">
      {@render icon('<rect x="3" y="3" width="18" height="18" rx="2" ry="2" /><circle cx="8.5" cy="8.5" r="1.5" /><polyline points="21 15 16 10 5 21" />')}
    </button>
    <button class="toolbar-btn" onclick={() => run(insertTable)} title="Table" aria-label="Insert table">
  ...
  <!-- Blocks -->
  <div class="toolbar-group">
    <button class="toolbar-btn" onclick={() => run(insertBlockquote)} title="Blockquote" aria-label="Blockquote">
    ...
    <button class="toolbar-btn" onclick={() => run(insertCodeBlock)} title="Code Block" aria-label="Code block">
      {@render icon('<rect x="3" y="3" width="18" height="18" rx="2" /><path d="M9 9l-2 3 2 3" /><path d="M15 9l2 3-2 3" />')}
    </button>
    <button class="toolbar-btn" onclick={() => run(insertHorizontalRule)} title="Horizontal Rule" aria-label="Horizontal rule">
  ...
  <!-- Lists -->
  <div class="toolbar-group">
    <button class="toolbar-btn" onclick={() => run(insertBulletList)} title="Bullet List" aria-label="Bullet list">
    ...
    <button class="toolbar-btn" onclick={() => run(insertNumberedList)} title="Numbered List" aria-label="Numbered list">
    ...
    <button class="toolbar-btn" onclick={() => run(insertTaskList)} title="Task List" aria-label="Task list">
